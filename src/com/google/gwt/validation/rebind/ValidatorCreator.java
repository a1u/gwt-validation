...
public class ValidatorCreator {
    ...
    public String createValidatorImplementation() {
        ...
        try {
            ...
            try {
                ...
                // getGroupSequenceMapping(Class<?> inputClass) {
                ...
                // write
                // "Set<InvalidConstraint> performValidation(T object, String propertyName, String... groups);"
                // method
                ...
                sw.println("if(propertyName != null && (propertyName.trim().length() == 0)) {"); // there
                // is
                // no
                // String.isEmpty()
                // in
                ...
                // get related classes
                final Set<JClassType> relatedClasses = JClassTypesHelper.getRelatedClasses(classType);

                for (final JClassType relatedClass : relatedClasses)
                    generateValidatorsForClass(sw, fullClass, relatedClass);
    ...
    /** Generate validator source for specified class 
     * @param sw source writer
     * @param fullClass parent class name
     * @param clazz field class 
     * @throws NotFoundException
     */
    private void generateValidatorsForClass(final SourceWriter sw, final String fullClass, final JClassType clazz) throws NotFoundException {

        Class<?> inputClass;
        try {
            inputClass = Class.forName(clazz.getQualifiedSourceName());
        } catch (final ClassNotFoundException e) {
        ...
        // if object is instance of related class
        sw.println("if( object instanceof " + clazz.getQualifiedSourceName() + " ) {");
        ...
        for (final ValidationPackage vPack : validationPackageList) {
            ...
            // create if string for the if block (includes
            // propertyName
            // and groups)
            ...
            // close if test
            ifTest += "))";
            // replace ' && ))' with '))' because I can't think of a
            // better
            // way to not have an && at the end of the series.
            ifTest = ifTest.replace(" && ))", "))");

            // create if block (used to break up scope and to test
            // for
            // propertyName / groups)
            ...
            // add value
            sw.println("ivc.setValue(((" + inputClass.getName() + ")object)." + vPack.getMethod().getName() + "());");
        ...
        for (final ValidationPackage vPack : vpValidList) {
            ...
            try {
                ...
                if (returnType != null) {
                    ...
                    // build if test and check that property name
                    // and
                    // object.method() are okay
                    ...
                    try {
                        if (returnType.asSubclass(Object[].class) != null) {
                            ...
                            typeName = typeName.substring(typeName.indexOf(" ")).trim();

                            // dont bork the whole thing on brittle
                            // failure
                            sw.println("try {");
                            sw.indent();
                            ...
                            // make sure it isn't part of the
                            // processed
                            // objects list
                            ...
                            // validate the object
                            sw.println("iCSet.addAll(this.unrollConstraintSet(object," + "\"" + vPack.getItemName() + "[\" + String.valueOf(it) + \"]\"" + // insert
                                    // sequence
                                    // no
                                    // from
                                    // collection
                            ...
                            sw.println("ex.printStackTrace();");
                            sw.outdent();
                            sw.println("}");

                            // has been validated somehow, no other
                            // method needed
                            validated = true;
                        }
                    ...
                    if (!validated) {
                        ...
                        try {
                            ...
                            if (collectionClass != null) {
                                ...
                                // this.typeOracle.getType(typeName);
                                // final JClassType[] subtypes =
                                // type.getSubtypes();
                                // for (final JClassType t : subtypes) {
                                // System.out.println(t.getQualifiedSourceName());
                                //
                                // }
                    ...
                    if (!validated) {
                        try {
                            ...
                            if (mapClass != null) {
                                ...
                                keyTypeName = keyTypeName.substring(keyTypeName.indexOf(" ")).trim();
                                ...
                                // validate the object
                                sw.println("iCSet.addAll(this.unrollConstraintSet(object, " + "\"" + vPack.getItemName() + "[\\\"\" + mapKey+ \"\\\"]\"" + // insert
                                        // map
                                        // key
                                        // into
                                        // []
                    ...
                    if (!validated) {
                        ...
                        // make sure it isn't part of the processed
                        // objects list
                        sw.println("if(!processedObjects.contains(objIdent)) {");
                        sw.indent();
        ...
        // close if - object is instance of related class
        sw.println("}");
    ...
    private SourceWriter getSourceWriter(final JClassType classType) {
        ...
        // create print writer for the given source, if the print writer is null
        // it either indicates
        // a problem with the filesystem or a that the class had already been
        // created and does not
        // need to be created again. in either case a null sourcewriter would be
        // created and the
        // creation method would return just the name that should have been used
        // so that it can
        ...
        if (printWriter != null) {
            ...
            // this.logger.log(TreeLogger.INFO,
            // "PrintWriter was null so the validator class was most likely initialized already.");
        }

        // logging
        // this.logger.log(TreeLogger.INFO, "Validator dynamically created for "
        // + fullName);
