...
public class ValidationJsr303MetadataFactory {
	...
	public static List<ValidationPackage> getValidatorsForClass(Class<?> inputClass){
				...
				for(Annotation annotationOnField : field.getAnnotations()) {
					...
					if(annotationArray.length > 0) {
						...
						for(Annotation annotationElement : annotationArray) {
							...
							Annotation checkTypeForConstraintValidator = annotationElement.annotationType().getAnnotation(Constraint.class);
					...
					//otherwise process as normal
						...
						//find a ConstraintValidator annotation on the given annotation type (for the annotation on the field)!
						Annotation checkTypeForConstraintValidator = annotationOnField.annotationType().getAnnotation(Constraint.class);
				...
				for(Annotation annotationOnMethod : method.getAnnotations()) {
					...
					if(annotationArray.length > 0) {
						...
						for(Annotation annotatedElement : annotationArray) {
							...
							//find a ConstraintValidator annotation on the given annotation type (for the annotation on the field)!
							Annotation checkTypeForConstraintValidator = annotatedElement.annotationType().getAnnotation(Constraint.class);
					...
					//otherwise continue processing the annotation as normal	
						...
						//find a ConstraintValidator annotation on the given annotation type (for the annotation on the field)!
						Annotation checkTypeForConstraintValidator = annotationOnMethod.annotationType().getAnnotation(Constraint.class);
				...
				if(annotationListForMethod != null && annotationListForMethod.size() > 0) {
					...
					for(Annotation annotation : annotationListForMethod) {
						...
						try {
							...
							if(simpleName.contains("Size")){
								validatorClass = new SizeValidator().getClass();
							}
							//cv = annotation.annotationType().getAnnotation(Constraint.class);
...
//							Class<?> validatorClass = (Class<?>) valueMethod.invoke(cv);
						...
						try {
							...
							Object invoke = groupMethod.invoke(annotation, new Object[]{});
							groups = (Class[]) groupMethod.invoke(annotation, new Object[]{});							
						} catch (SecurityException e) {
							//e.printStackTrace();
						} catch (NoSuchMethodException e) {
							//e.printStackTrace();
						...
						if(constraint != null) {
							...
							ValidationPackage vPackage = new ValidationPackage(constraint, message, method, field, groups2, propertyMap);
